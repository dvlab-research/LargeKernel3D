...
class ChromaticTranslation(object):
  ...
  def __init__(self, trans_range_ratio=1e-1, use_ori_coords=False):
    """
    trans_range_ratio: ratio of translation i.e. 255 * 2 * ratio * rand(-0.5, 0.5)
    """
    self.trans_range_ratio = trans_range_ratio
    self.use_ori_coords = use_ori_coords

  def __call__(self, *inputs):
    if self.use_ori_coords:
      coords, feats, labels, ori_coords = inputs
    else:
      coords, feats, labels = inputs
    if random.random() < 0.95:
      tr = (np.random.rand(1, 3) - 0.5) * 255 * 2 * self.trans_range_ratio
      feats[:, :3] = np.clip(tr + feats[:, :3], 0, 255)
    if self.use_ori_coords:
      return coords, feats, labels, ori_coords
    return coords, feats, labels

class ChromaticTranslationDistill(object):
  """Add random color to the image, input must be an array in [0,255] or a PIL image"""
  def __init__(self, trans_range_ratio=1e-1, use_ori_coords=False):
    """
    trans_range_ratio: ratio of translation i.e. 255 * 2 * ratio * rand(-0.5, 0.5)
  ...
  def __call__(self, *inputs):
    if self.use_ori_coords:
      coords, feats, labels, ori_coords, coords_t, feats_t, labels_t, ori_coords_t = inputs
    else:
      coords, feats, labels, coords_t, feats_t, labels_t = inputs
    if random.random() < 0.95:
      tr = (np.random.rand(1, 3) - 0.5) * 255 * 2 * self.trans_range_ratio
      feats[:, :3] = np.clip(tr + feats[:, :3], 0, 255)
      feats_t[:, :3] = np.clip(tr + feats_t[:, :3], 0, 255)
    if self.use_ori_coords:
      return coords, feats, labels, ori_coords, coords_t, feats_t, labels_t, ori_coords_t
    return coords, feats, labels, coords_t, feats_t, labels_t


class ChromaticAutoContrast(object):

  def __init__(self, randomize_blend_factor=True, blend_factor=0.5, use_ori_coords=False):
...
class ChromaticAutoContrastDistill(object):
  ...
  def __call__(self, *inputs):
    ...
    if random.random() < 0.2:
      ...
      lo_t = feats_t[:, :3].min(0, keepdims=True)
      hi_t = feats_t[:, :3].max(0, keepdims=True)
      
      # if hi.max() <= 1:
      # print("*********************hi***********************", hi.max(), coords.max())
      assert hi.max() > 1, f"invalid color value. Color is supposed to be [0-255]"
      assert hi_t.max() > 1, f"invalid color value. Color is supposed to be [0-255]"

      scale = 255 / (hi - lo)
      scale_t = 255 / (hi_t - lo_t)

      contrast_feats = (feats[:, :3] - lo) * scale
      contrast_feats_t = (feats_t[:, :3] - lo_t) * scale_t

      blend_factor = random.random() if self.randomize_blend_factor else self.blend_factor
      feats[:, :3] = (1 - blend_factor) * feats + blend_factor * contrast_feats
      feats_t[:, :3] = (1 - blend_factor) * feats_t + blend_factor * contrast_feats_t
    if self.use_ori_coords:
      return coords, feats, labels, ori_coords, coords_t, feats_t, labels_t, ori_coords_t
    return coords, feats, labels, coords_t, feats_t, labels_t
...
class ChromaticJitter(object):
  ...
  def __init__(self, std=0.01, use_ori_coords=False):
    self.std = std
    self.use_ori_coords = use_ori_coords
...
class ChromaticJitterDistill(object):
  ...
  def __call__(self, *inputs):
    ...
    if random.random() < 0.95:
      ...
      noise_t = np.random.randn(feats_t.shape[0], 3)
      noise_t *= self.std * 255
      feats_t[:, :3] = np.clip(noise_t + feats_t[:, :3], 0, 255)             
    if self.use_ori_coords:
      return coords, feats, labels, ori_coords, coords_t, feats_t, labels_t, ori_coords_t
    return coords, feats, labels, coords_t, feats_t, labels_t
...
class RandomDropout(object):
  ...
  def __init__(self, dropout_ratio=0.2, dropout_application_ratio=0.5, use_ori_coords=False):
    """
  ...
  def __call__(self, *inputs):
    ...
    if random.random() < self.dropout_ratio:
      ...
      if self.use_ori_coords:
        return coords[inds], feats[inds], labels[inds], ori_coords[inds]
      return coords[inds], feats[inds], labels[inds]
    if self.use_ori_coords:
      return coords, feats, labels, ori_coords
    return coords, feats, labels

class RandomDropoutDitill(object):
  ...
  def __init__(self, dropout_ratio=0.2, dropout_application_ratio=0.5, use_ori_coords=False):
    ...
    self.dropout_application_ratio = dropout_application_ratio
    self.use_ori_coords = use_ori_coords
    # self.use_same_transform =  use_same_transform
  ...
  def __call__(self, *inputs):
    ...
    if random.random() < self.dropout_ratio:
      ...
      M = len(coords_t)
      inds = np.random.choice(N, int(N * (1 - self.dropout_ratio)), replace=False)
      inds_t = np.random.choice(M, int(M * (1 - self.dropout_ratio)), replace=False)
      if self.use_ori_coords:
        return coords[inds], feats[inds], labels[inds], ori_coords[inds], coords_t[inds_t], feats_t[inds_t], labels_t[inds_t], ori_coords_t[inds_t]
      else:
        return coords[inds], feats[inds], labels[inds], coords_t, feats_t, labels_t
    if self.use_ori_coords:
      return coords, feats, labels, ori_coords, coords_t, feats_t, labels_t, ori_coords_t
    else:
      return coords, feats, labels, coords_t, feats_t, labels_t
...
class RandomHorizontalFlip(object):
  ...
  def __init__(self, upright_axis, is_temporal, use_ori_coords=False):
    """
    upright_axis: axis index among x,y,z, i.e. 2 for z
    """
...
class RandomHorizontalFlipDistill(object):
    ...
    if random.random() < 0.95:
      for curr_ax in self.horz_axes:
        if random.random() < 0.5:
          coord_max = np.max(coords[:, curr_ax])
          coords[:, curr_ax] = coord_max - coords[:, curr_ax]
          coord_max_t = np.max(coords_t[:, curr_ax])
          coords_t[:, curr_ax] = coord_max_t - coords_t[:, curr_ax]
    if self.use_ori_coords:
      return coords, feats, labels, ori_coords, coords_t, feats_t, labels_t, ori_coords_t
    return coords, feats, labels, coords_t, feats_t, labels_t
