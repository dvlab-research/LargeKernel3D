...
class RandomScaleV2(object):
    ...
    def __repr__(self):
        # return 'RandomScale(scale_low: {}, scale_high: {})'.format(self.scale_low, self.scale_high)
        return 'RandomScale(scale: {})'.format(self.scale)
...
class RandomShiftV2(object):
    def __init__(self, shift_size=0.1):
        self.shift_size = shift_size

    def __call__(self, points, color):
        # shift = np.random.uniform(-self.shift_range, self.shift_range, 3)
        points[:, 0:3] += self.shift_size
        return points, color

    def __repr__(self):
        return 'RandomShift(shift_size: {})'.format(self.shift_size)
